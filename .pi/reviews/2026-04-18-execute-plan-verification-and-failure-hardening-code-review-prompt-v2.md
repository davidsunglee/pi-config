<!-- Review dispatch prompt for iteration 2 hybrid re-review -->
# Code Review Agent
...
```bash
git diff --stat 8e6ca8b..97cffe41dbd9e760263cd7d621de6eefe3566b3e
git diff 8e6ca8b..97cffe41dbd9e760263cd7d621de6eefe3566b3e
```
...
1. **Incomplete Step 10 criterion coverage validation**
   - **File:** `agent/skills/execute-plan/SKILL.md:648-660`
   - **What's wrong:** Step 10.3 validates the shape of verifier output, but it never requires a verdict block for every acceptance criterion. A verifier response that omits Criterion 2+ and still says `VERDICT: PASS` would satisfy the documented parsing rules.
   - **Why it matters:** This breaks the core requirement that verification return per-criterion judgments and that every criterion be binary/pass-fail. Missing criterion verdicts can let incomplete verification slip through as a pass.
   - **How to fix:** Require Step 10.3 to compare the number/set of `[Criterion N]` blocks against the task’s acceptance-criteria count, and treat any missing, duplicated, or out-of-range criterion numbers as protocol errors routed to Step 12.

2. **Baseline capture still allows heuristic failure-set tracking**
   - **File:** `agent/skills/execute-plan/SKILL.md:279-291,710-736`
   - **What's wrong:** Step 11’s new three-set model depends on exact failing-test identifier sets, but Step 7 still allows a “simple heuristic” based on counts or ad hoc name comparison for baseline capture.
   - **Why it matters:** Equal-count swaps, renamed failures, or inconsistent parsing will corrupt `baseline_failures`, which then cascades into wrong `deferred_integration_regressions` / `new_regressions_after_deferment` classification. That undermines the very failure-hardening this change is supposed to provide.
   - **How to fix:** Update Step 7 to require extraction and storage of the same suite-native failing-test identifiers that Step 11 uses later. Remove the heuristic language and make Step 7/11 use one consistent identifier-parsing rule.

3. **Three headed integration sections are only mandatory on the fail path**
   - **File:** `agent/skills/execute-plan/SKILL.md:740-760`
   - **What's wrong:** The three headed sections (`Baseline failures`, `Deferred integration regressions`, `New regressions in this wave`) are only required on the fail path when `new_regressions_after_deferment` is non-empty. On the pass path, the skill only says to “note” remaining baseline/deferred failures briefly.
   - **Why it matters:** The requirements explicitly say that when integration tests are not clean, user-facing summaries must present all three sets separately with explicit headings. As written, later waves can still have unresolved deferred regressions but no required headed summary, which makes those regressions less visible.
   - **How to fix:** Require the same three-section summary whenever `current_failing` is non-empty, even if classification is “pass (no new regressions)”. The only difference between pass/fail should be the menu, not the visibility of the tracked sets.

4. **Debugger-first success criteria collapse deferred-regression tracking**
   - **File:** `agent/skills/execute-plan/SKILL.md:805-808`
   - **What's wrong:** The debugger-first flow still treats success as “matches the baseline” / “passes” after re-running tests. That is incompatible with the new deferred-regression model, where previously deferred regressions may legitimately remain failing mid-run.
   - **Why it matters:** After a user has deferred regressions in an earlier wave, fixing only the new regressions in a later wave should be enough to continue. The current wording instead forces the suite back to baseline/clean before proceeding, which collapses the intended defer-and-continue path.
   - **How to fix:** After a debug/remediation attempt, re-run the Step 11 reconciliation logic and continue when `new_regressions_after_deferment` is empty. Preserve and report any still-failing deferred regressions instead of requiring baseline equality.
