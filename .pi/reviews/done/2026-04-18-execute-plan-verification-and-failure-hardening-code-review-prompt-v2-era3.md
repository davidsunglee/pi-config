<!-- Review dispatch prompt for era 3 hybrid re-review -->
# Code Review Agent
...
**Base:** 14a898829badf056dfdbb8afcde3f2f2b143d7fe
**Head:** bb5195d

```bash
git diff --stat 14a898829badf056dfdbb8afcde3f2f2b143d7fe..bb5195d
git diff 14a898829badf056dfdbb8afcde3f2f2b143d7fe..bb5195d
```
...
1. **Verifier contract still contradicts the new authoritative file-set model**
   - **File:** `agent/agents/verifier.md:11,30,37`
   - **What's wrong:** The verifier agent still says it receives `## Modified Files`, describes that section as “the list of files the worker changed,” and forbids reading outside that worker-changed set. The new execution flow and prompt template now use `## Verifier-Visible Files` as an orchestrator-assembled union of task-declared scope, worker report, and observed diff state.
   - **Why it matters:** One of the main hardening goals was preventing a worker from narrowing its own verification surface. Leaving the verifier’s base contract on the old “worker changed these files” model creates conflicting instructions in the most safety-critical part of the flow. In practice that can cause the verifier to under-read declared files, mis-handle untouched-but-in-scope files, or treat the new prompt as inconsistent.
   - **How to fix:** Update `agent/agents/verifier.md` to use `## Verifier-Visible Files` terminology throughout and explicitly describe it as the orchestrator-authored authoritative union, not a worker self-report. Align any dependent wording that still references `## Modified Files`.

2. **The Step 15 “debug now” path reuses a wave-scoped debugging flow that is not executable at final-gate time**
   - **File:** `agent/skills/execute-plan/SKILL.md:831-838,957-964`
   - **What's wrong:** Step 15 tells the orchestrator to run the Step 11 debugger-first flow against `deferred ∪ new` regressions, but that flow is defined in terms of current-wave artifacts: “the wave commit SHA,” `git show HEAD` for the wave commit, and a suspect task list built from the current wave’s tasks. At final completion time, deferred regressions may come from earlier waves and fresh regressions may come from Step 14 review/remediation commits, so there is no single current-wave context to plug into that flow.
   - **Why it matters:** At the final gate, the only productive path besides stopping is “Debug failures now.” If that path depends on wave-local state that no longer exists, the workflow becomes under-specified exactly where the branch is blocked from completion. That is a production-readiness problem for the new final-gate contract.
   - **How to fix:** Define a Step 15-specific debugging flow, or persist enough provenance during deferment/review remediation (originating wave, implicated tasks, relevant commit SHAs/files) so the final gate can build a concrete debugging prompt without assuming a current-wave context.
