<!-- Review dispatch prompt for era 2 hybrid re-review -->
# Code Review Agent
...
```bash
git diff --stat 97cffe41dbd9e760263cd7d621de6eefe3566b3e..14a898829badf056dfdbb8afcde3f2f2b143d7fe
git diff 97cffe41dbd9e760263cd7d621de6eefe3566b3e..14a898829badf056dfdbb8afcde3f2f2b143d7fe
```
...
1. **Final completion can still report success after post-wave review/remediation introduces brand-new integration regressions.**
   - **File:** `agent/skills/execute-plan/SKILL.md:886-908, 916-953`
   - **What's wrong:** Step 14 allows `refine-code` to make additional code changes after the final wave, but Step 15’s “always run” integration rerun only reconciles `deferred_integration_regressions` and gates on `still_failing_deferred`. It never classifies or blocks on fresh regressions that appear at this point and were not previously deferred.
   - **Why it matters:** This violates the hardening goal that integration failures must not silently degrade into a nominally successful run. A review remediation can break the suite, Step 15 can still pass, and the workflow can move the plan to `done/`, close the todo, and enter branch-completion flow while the branch is newly failing.
   - **How to fix:** In Step 15, after rerunning the suite, compute the same classification used in Step 11 (`baseline_failures`, reconciled `deferred_integration_regressions`, and `new_regressions_after_deferment`) and block completion on any non-baseline regression, not just still-failing deferred ones. Reuse the final-wave debug/stop handling for newly discovered regressions.

2. **The verifier’s evidence boundary is still controlled by the worker being judged.**
   - **File:** `agent/skills/execute-plan/SKILL.md:649-651`; `agent/skills/execute-plan/verify-task-prompt.md:23-31, 43-46`
   - **What's wrong:** Step 10.2 builds both `{MODIFIED_FILES}` and `{DIFF_CONTEXT}` from the coder’s self-reported `## Files Changed` section, and the verifier prompt then forbids reading outside that list unless a recipe explicitly names another file.
   - **Why it matters:** This weakens the “fresh-context independent verifier” boundary. If the worker omits a changed file—accidentally or otherwise—the verifier loses both the file list and the diff for that change, so hidden side effects can escape verification entirely.
   - **How to fix:** Derive the verification file set from orchestrator-observed state, not just worker self-report. At minimum, pass the union of the task’s declared `**Files:**` entries and actual changed files attributable to that task; ideally, capture per-task file changes in isolation so the verifier’s input cannot be narrowed by the worker.
