...
- `agent/agents/plan-reviewer.md` (Modify) — Update the Principles bullet "Give a clear verdict — always conclude with `[Approved]` or `[Issues Found]`" to reference the three new verdict labels, the `**Verdict:**` line, and the `### Outcome` section. Update the Principles "Calibrate severity" bullet, the Rules "use severity levels accurately" bullet, and the `## Approach honoring` `**Severity:**` paragraph so they use Critical / Important / Minor vocabulary instead of Error / Warning / Suggestion. Update the Output Artifact Contract Step 2's body description from "Status verdict, Issues, Summary" to "Outcome, Strengths, Issues, Recommendations".
...
- `agent/agents/plan-refiner.md` (Modify) — No verdict-vocabulary references in current text, but the role bullet 5 says "Append warnings/suggestions to the plan as `## Review Notes` only on the approved path". Update to "Append the waived-Important pointer block to the plan as `## Review Notes` only on the `approved_with_concerns` path". Update bullet 3 reference to "Status line" to "Verdict line". Update the Rules bullet "every error finding feeds the single planner edit pass for that iteration" so it qualifies the edit-pass scope to `Not approved` outcomes (Critical and Important findings the reviewer judged blocking) and explicitly excludes Important findings the reviewer waived under `Approved with concerns`. Update "return `issues_remaining`" to "return `not_approved_within_budget`".
...
- [ ] **Step 3: Update plan-reviewer legacy severity vocabulary.** In `agent/agents/plan-reviewer.md`, audit and update three places that still use Error / Warning / Suggestion vocabulary so they align with Critical / Important / Minor:
  - Principles bullet "**Calibrate severity** — a vague acceptance criterion is a Warning, a missing task is an Error. Do not inflate." → Replace with "**Calibrate severity** — a vague acceptance criterion is Important, a missing task is Critical. Do not inflate."
  - Rules bullet "Do NOT mark everything as an error — use severity levels accurately (Error, Warning, Suggestion)" → Replace with "Do NOT mark everything as Critical — use severity levels accurately (Critical, Important, Minor)".
  - `## Approach honoring` `**Severity:**` paragraph: replace "every deviation from the spec's chosen approach is flagged as a **Warning** — never downgraded to a Suggestion, never omitted." with "every deviation from the spec's chosen approach is flagged as **Important** — never downgraded to Minor, never omitted." In the same paragraph, replace "cite that justification inside the Warning so the user can see both the deviation and its rationale" with "cite that justification inside the Important finding so the user can see both the deviation and its rationale", and replace "The presence of a `Risk Assessment` entry does not suppress the Warning — surfacing the deviation is the contract that keeps the user's chosen approach visible end-to-end." with "The presence of a `Risk Assessment` entry does not suppress the Important finding — surfacing the deviation is the contract that keeps the user's chosen approach visible end-to-end."

- [ ] **Step 4: Update code-reviewer Principles bullet.** In `agent/agents/code-reviewer.md`, locate the Principles bullet "**Give a clear verdict** — always include a 'Ready to merge: Yes/No/With fixes' line in the Assessment section." Replace with "**Give a clear verdict** — always emit a `**Verdict:**` line with one of `Approved`, `Approved with concerns`, or `Not approved` in the `### Outcome` block at the top of your review, followed by the `**Reasoning:**` line. Critical findings always force `Not approved`; `Approved with concerns` is allowed only when zero Critical findings exist and one or more Important findings are explicitly waived in the Reasoning line."

- [ ] **Step 5: Update code-reviewer Output Artifact Contract Step 2 wording.** Locate Step 2 of the Output Artifact Contract: "The provenance line is followed by a single blank line, then the review body (Strengths, Issues, Recommendations, Assessment as defined in your prompt template's Output Format)." Replace "(Strengths, Issues, Recommendations, Assessment as defined in your prompt template's Output Format)" with "(Outcome, Strengths, Issues by severity, Recommendations as defined in your prompt template's Output Format)".

- [ ] **Step 6: Update plan-refiner role bullets.** In `agent/agents/plan-refiner.md`, locate the `## Your Role` numbered list. Update bullet 3 from "**Parse** the Status line and findings from the on-disk review" to "**Parse** the Verdict line and findings from the on-disk review". Update bullet 5 from "**Append** warnings/suggestions to the plan as `## Review Notes` only on the approved path (this is an edit to the PLAN file, not to the reviewer artifact)" to "**Append** the waived-Important pointer block to the plan as `## Review Notes` only on the `approved_with_concerns` path (this is an edit to the PLAN file, not to the reviewer artifact)".

- [ ] **Step 7: Audit plan-refiner Rules and Boundary sections.** In `agent/agents/plan-refiner.md`, scan the `## Rules` and `## Boundary with refine-plan` sections for any lingering references to `Errors`, `Warnings`, `Suggestions`, `[Approved]`, `[Issues Found]`, `issues_remaining`, or `STATUS: approved` / `STATUS: issues_remaining` / `STATUS: failed` outcomes. The rules use phrases like "every error finding feeds the single planner edit pass for that iteration" — update with the qualified replacement "on `Not approved` outcomes, every Critical and Important finding the reviewer judged blocking feeds the single planner edit pass for that iteration; on `Approved with concerns` outcomes the refiner exits on the success path and does NOT dispatch a planner edit pass (Important findings waived by the reviewer are final and never feed an edit pass)". Update "return `issues_remaining` when the budget for this era is exhausted" to "return `not_approved_within_budget` when the budget for this era is exhausted".

- [ ] **Step 8: Audit code-refiner Rules section.** In `agent/agents/code-refiner.md`, scan the `## Rules` section for any lingering verdict-vocabulary references requiring updates. The current text has "Do NOT ignore Critical or Important findings — they must be addressed or escalated" — this is a severity reference, not an outcome reference, and stays as-is. Confirm by reading the entire `## Rules` block and confirming no references to `[Approved]`, `[Issues Found]`, `Ready to merge`, `STATUS: clean`, `STATUS: max_iterations_reached`, `Errors`, `Warnings`, or `Suggestions` exist. If any do, update per the new vocabulary.
...
- plan-reviewer Principles "Calibrate severity" bullet uses Critical / Important vocabulary.
  Verify: open `agent/agents/plan-reviewer.md` `## Principles` section and confirm the "Calibrate severity" bullet contains the substrings "is Important" and "is Critical"; `grep -n "is a Warning, a missing task is an Error" agent/agents/plan-reviewer.md` returns zero matches.
- plan-reviewer Rules severity-levels bullet uses Critical / Important / Minor vocabulary.
  Verify: open `agent/agents/plan-reviewer.md` `## Rules` section and confirm the severity-levels bullet contains the substring `(Critical, Important, Minor)`; `grep -n "(Error, Warning, Suggestion)" agent/agents/plan-reviewer.md` returns zero matches.
- plan-reviewer Approach honoring severity rule uses Important / Minor vocabulary.
  Verify: open the `## Approach honoring` `**Severity:**` paragraph in `agent/agents/plan-reviewer.md` and confirm it states the deviation is flagged as `**Important**` (not `**Warning**`) and references downgrade to `Minor` (not `Suggestion`); `grep -nE "flagged as a \\*\\*Warning\\*\\*|downgraded to a Suggestion|inside the Warning|suppress the Warning" agent/agents/plan-reviewer.md` returns zero matches.
...
- plan-refiner Rules use the new severity vocabulary AND qualify edit-pass scope to `Not approved` outcomes only.
  Verify: open the `## Rules` section in `agent/agents/plan-refiner.md` and confirm the edit-pass-scope bullet contains the substrings `Not approved`, `Critical and Important`, and a phrase that explicitly excludes `Approved with concerns` waivers from feeding an edit pass (e.g. "does NOT dispatch" or "never feed" alongside `Approved with concerns`); `grep -nE "every error finding feeds" agent/agents/plan-refiner.md` returns zero matches.
