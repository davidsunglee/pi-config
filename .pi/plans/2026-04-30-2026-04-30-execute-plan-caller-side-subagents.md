...
- [ ] **Step 9: Self-review SKILL.md end-to-end.**
  ...
  - Confirm the file's total line count is `<=` the branch-start / pre-plan line count. Use the branch base recorded by the plan executor (preferred) or compute it from the merge-base with the base branch before comparing. Do NOT derive the baseline from the oldest reachable commit that ever modified `agent/skills/execute-plan/SKILL.md`.
...
- Net `agent/skills/execute-plan/SKILL.md` line count is less than or equal to its branch-start / pre-plan line count.
  Verify: run `git rev-parse HEAD` to confirm the current branch contains all of Tasks 1-7's commits, then compute the baseline from the branch base (not from the oldest reachable commit that ever modified `SKILL.md`). Prefer an executor-recorded `PLAN_BASE_REF` / `PLAN_BASE_SHA` if available; otherwise use the merge-base with the configured base branch. Compute `BASE_LINE_COUNT = $(git show <branch-base-sha>:agent/skills/execute-plan/SKILL.md | wc -l)`, compute `CURRENT_LINE_COUNT = $(wc -l < agent/skills/execute-plan/SKILL.md)`, and confirm `CURRENT_LINE_COUNT -le BASE_LINE_COUNT`. Concretely, run this verification:
  
  ```bash
  BASE_BRANCH=${PLAN_BASE_REF:-origin/main}
  BASE=${PLAN_BASE_SHA:-$(git merge-base HEAD "$BASE_BRANCH")}
  ...
  Confirm the script prints `OK`. If the repository has no `origin/main` in the execution environment, set `PLAN_BASE_REF` or `PLAN_BASE_SHA` explicitly before running the verification.
...
| Net SKILL.md line count exceeds the pre-branch baseline because Task 6 and Task 7 add more prose than they remove. | Low | Medium | Task 7 Step 9 compares the final SKILL.md line count against the branch-start / pre-plan line count using an executor-recorded base SHA when available, otherwise `git merge-base HEAD ${PLAN_BASE_REF:-origin/main}`. It explicitly forbids deriving the baseline from the oldest reachable commit that ever modified SKILL.md. The plan removes Step 11.1 in Task 6 (≈14 lines), the Step 7 identifier-extraction subsection in Task 7 (≈25 lines), the Step 12.2 bash block (≈8 lines), and inline test-command shell at the Debugger-first / Step 16 sites (≈5 lines), totaling ≈52 lines removed. The shared `#### Test-runner dispatch (shared)` subsection adds ≈40 lines once and is referenced by all four call sites; per-site additions are small (≈5 lines each). Net change is expected to be slightly negative, comfortably within the `<=` constraint. |
