...
### `pi-interactive-subagent` frontmatter and prompt-delivery contract

Per the `pi-interactive-subagent` frontmatter contract used by this project, supported fields include `name`, `description`, `model`, `cli`, `thinking`, `tools`, `skills`, `system-prompt`, `session-mode`, `spawning`, `deny-tools`, `auto-exit`, `cwd`, and `disable-model-invocation`. **`maxSubagentDepth` is not in this list.** All six existing agents in `agent/agents/` (planner, plan-reviewer, coder, code-reviewer, code-refiner, verifier) currently set `maxSubagentDepth`, which is silently ignored by the runtime. None of the existing agents currently set `session-mode`, so they all default to `standalone` rather than `lineage-only`.

There is one load-bearing runtime constraint for `spec-designer`: the current subagent runtime resolves the child identity prompt as `agentDefs.body ?? params.systemPrompt`. A non-empty body in `agent/agents/spec-designer.md` would therefore shadow the per-call `systemPrompt:` that carries `procedure.md`. To keep `procedure.md` as the single canonical procedure on both pi and Claude paths, `spec-designer.md` must be **frontmatter-only** and must opt in to appended per-call system prompts with `system-prompt: append`.
...
- The orchestrator's branch decision collapses to **`mux` | `inline`**. On the `mux` branch, the orchestrator also resolves the dispatch `model` and `cli` explicitly from `~/.pi/agent/model-tiers.json`: use the `capable` tier as the per-call `model`, then use that model's provider prefix to look up `dispatch.<provider>` as the per-call `cli`. This preserves the current Opus-tier / Claude-CLI routing while avoiding a `model:` field in `spec-designer.md`. This is not a separate procedure branch — the same dispatch shape and procedure body are used regardless of which CLI is resolved.
- For both branches, the procedure body lives in exactly one file (`agent/skills/define-spec/procedure.md`). Delivery differs per branch:
  - **`mux` branch:** orchestrator reads `procedure.md`, resolves `model` + `cli` from `model-tiers.json`, and dispatches `spec-designer` via `subagent_run_serial` (blocking, `wait: true`) with `systemPrompt: <procedure body>`, `task: <raw input>`, `agent: spec-designer`, `model: <capable tier>`, and `cli: <resolved dispatch CLI>`. **No `skills:` parameter is passed** — the procedure is delivered via `systemPrompt:` on both pi and Claude CLI paths so the delivery mechanism is symmetric.
  - **`inline` branch:** orchestrator reads `procedure.md` and runs the procedure in its own session. It does **not** resolve `model` or `cli`, because there is no subagent dispatch boundary; the procedure runs under the already-active orchestrator session/model.
- The orchestrator must read `procedure.md` fresh at dispatch time. If the file is unreadable, fail cleanly with a clear error rather than dispatching with an empty/truncated procedure. On the `mux` branch, it must also fail cleanly if `model-tiers.json` cannot be read or does not provide a usable `capable` model / `dispatch.<provider>` mapping.
...
- The orchestrator reads no project/domain files other than `procedure.md` (the procedure body it dispatches). On the `mux` branch only, it may also read `~/.pi/agent/model-tiers.json` to resolve the per-call `model` and `cli` required for dispatch.
...
- **`agent/agents/plan-reviewer.md` body.** When the plan-reviewer reads both spec and plan, if the spec has a `## Approach` section, the review must check the plan honors it. **Deviations are flagged as Warnings.** If the section is absent, current behavior is preserved.
...
- `spec-designer` runs without a `model:` frontmatter field; the orchestrator passes explicit per-call `model` and `cli` values resolved from `model-tiers.json` on the `mux` branch.
...
- `spec-designer` must work end-to-end on **both** pi CLI and Claude CLI. The orchestrator resolves the per-call `cli` from `model-tiers.json`, but the dispatch shape is identical regardless of which CLI is selected.
...
**Chosen approach:** Three coordinated workstreams in one spec — (1) introduce `spec-designer` agent + procedure file + revised orchestrator skill, (2) normalize agent frontmatter across all seven agents, (3) expand the procedure to optionally walk a brainstorming-style architecture round, with the spec template gaining an optional `## Approach` section and `planner` / `plan-reviewer` updated to honor it. Procedure body is delivered via `systemPrompt:` on **both** pi and Claude CLI dispatch paths (no `skills:` mechanism used at all), giving a symmetric single-delivery-mechanism story. `spec-designer.md` is frontmatter-only with `system-prompt: append` so the runtime cannot shadow the per-call procedure with agent body text. On the mux branch, the orchestrator reads `model-tiers.json` and passes explicit per-call `model` + `cli` values so Opus-tier dispatch keeps routing through the intended CLI. Procedure file lives **inside** the `define-spec/` skill directory as a non-skill plain file, so there is no top-level skill ambiguity between `define-spec` and a procedure-skill sibling.
...
- **Frontmatter-only `spec-designer` + symmetric delivery via `systemPrompt:`** sidesteps both relevant runtime constraints: skills aren't forwarded to the Claude CLI, and a non-empty agent body would shadow `params.systemPrompt`. A two-mechanism design (pi `skills:` + Claude `systemPrompt:`) was viable but introduced asymmetry for no functional gain — both CLIs can receive the same procedure through `systemPrompt:` cleanly when `spec-designer.md` is body-less and declares `system-prompt: append`.
- **Explicit per-call `model` / `cli` resolution on the mux branch** keeps subagent dispatch aligned with `model-tiers.json`. Relying on agent frontmatter is intentionally avoided (`spec-designer.md` has no `model:`), and relying on runtime defaults risks losing the capable-tier / Claude-CLI route that motivated this split. The inline branch does not resolve `model` / `cli` because it performs no dispatch and simply continues in the current orchestrator session.
...
- **Use `pi-interactive-subagent`'s `skills:` mechanism on pi and inline only on Claude** — asymmetric and would bifurcate procedure delivery / testing for no functional gain. Rejected in favor of single delivery via `systemPrompt:` on both paths.
- **Adopt `superpowers:brainstorming` directly as the procedure** — rejected. The architecture-round expansion narrows the gap with brainstorming but does not close it. Brainstorming is a *design* skill that walks intent **and** full design (architecture, components, data flow, error handling, testing) and terminates by invoking `writing-plans`. `define-spec` after the expansion is a *spec* skill that captures intent **and** macro architectural choice (paradigm-level: subagent vs inline, monolith vs split, sync vs async, single-skill vs multi-skill), then stops — components, file structure, types, error-handling shape, and test design remain the `planner`'s job, downstream of `generate-plan`. Adopting brainstorming directly would either duplicate `planner.md`'s responsibilities (component / data-flow / file-structure design) or force `define-spec` into territory that belongs to deep codebase analysis. The other contract differences (architecture is *optional* in `define-spec` with explicit user-override on the load-bearing assessment; output path is `.pi/specs/`; terminal is `generate-plan`) compound the mismatch — adopting brainstorming would require enough local overrides to amount to maintaining a fork. Lifting brainstorming's *techniques* (one-question-at-a-time pacing, scope-decomposition check, propose-2–3-approaches round, spec self-review pass, user-review gate) into a procedure that respects `define-spec`'s contract is the right level of reuse.
