...
- The default color for the subscription sub-indicator should match the muted gray currently used for the model provider label
- Row 1 project identity formatting should remove parentheses around the git branch
- Row 1 should use a muted gray dot separator between `cwd` and git branch
- Row 1 should not render a trailing dot or equivalent separator after the git branch when no further left-side field follows it
...
- Cost and subscription indicator must behave as a single display unit; they appear together or disappear together, while still allowing the subscription sub-indicator to have its own color styling when shown
...
- The row 1 separator and branch presentation should remain visually clear without relying on parentheses for disambiguation
...
- The cost subscription sub-indicator can be styled independently through footer theme overrides, and by default uses the same muted gray as the model provider label
...
- Row 1 displays `cwd`, then a muted gray dot separator, then the git branch with no parentheses, and does not leave behind orphaned row-1 separators when fields are hidden
