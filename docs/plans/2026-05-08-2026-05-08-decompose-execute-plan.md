...
**Steps:**
- [ ] **Step 1: Capture predecessor checksum** — Run `shasum -a 256 agent/skills/define-spec/procedure.md` and record the hash so byte-identity can be re-verified after the rename.
- [ ] **Step 2: Rename via `git mv`** — Run `git mv agent/skills/define-spec/procedure.md agent/skills/define-spec/spec-design-procedure.md`. `git mv` is a byte-preserving move (no content edits). If the working tree is not a git checkout, fall back to `cp agent/skills/define-spec/procedure.md agent/skills/define-spec/spec-design-procedure.md && rm agent/skills/define-spec/procedure.md` (still byte-preserving via `cp`).
- [ ] **Step 3: Confirm byte-for-byte match against captured checksum** — Run `shasum -a 256 agent/skills/define-spec/spec-design-procedure.md` and confirm the hash equals the value recorded in Step 1. If it does not match, abort the task — the move corrupted content.
...
- The byte content of `spec-design-procedure.md` is identical to the predecessor (no content drift).
  Verify: in a git checkout, run `git diff -M --name-status --staged HEAD -- agent/skills/define-spec/` (after the `git mv` of Step 2 has staged the rename) and confirm the output contains a single line of the form `R100\tagent/skills/define-spec/procedure.md\tagent/skills/define-spec/spec-design-procedure.md` (100% rename similarity). If the executor used the `cp + rm` fallback, instead confirm that the SHA-256 captured in Step 1 equals the SHA-256 recorded in Step 3 (both hashes appear in the executor's transcript), and that `git diff -M50 HEAD -- agent/skills/define-spec/` reports a rename for these two paths.
- A repo-wide grep for `define-spec/procedure.md` (excluding spec/plan/todo history) returns zero matches in live source files.
  Verify: from the repo root, run `grep -rn "define-spec/procedure.md" . --exclude-dir=node_modules --exclude-dir=.git --include='*.md' --include='*.py' | grep -v -E "^(\\./)?(docs/specs|docs/plans|docs/todos)/"` and confirm zero matches (or non-zero exit). Allow matches inside `docs/specs/`, `docs/plans/`, `docs/todos/` since those are historical artifacts.
...
- [ ] **Step 10: Sanity-check verbatim preservation via section-by-section diff** — For each preserved section (`## Identifier contract`, `## Tracked state`, `## Per-run inputs and reconciliation`, `## Pass/fail classification`, `## User-facing summary format`, `## Worked examples`), extract the section's body from both `agent/skills/execute-plan/integration-regression-model.md` (predecessor) and `agent/skills/execute-plan/integration-regression-gate.md` (new file) using `awk '/^## Identifier contract$/,/^## /'` (and analogous patterns per heading), write each pair to temp files, and run `diff` between them. Every diff MUST exit with code 0 (empty output). If any diff produces output, fix the new file before proceeding — verbatim preservation is the contract. Record the diff invocations and their (empty) outputs in the task transcript so the verifier can confirm.
...
- Each preserved section's body in the new file is byte-for-byte identical to the predecessor's corresponding section.
  Verify: for each section name in `## Identifier contract`, `## Tracked state`, `## Per-run inputs and reconciliation`, `## Pass/fail classification`, `## User-facing summary format`, `## Worked examples`, run `awk '/^<HEADING>$/{flag=1;next} /^## /{flag=0} flag' agent/skills/execute-plan/integration-regression-model.md > /tmp/pred-<slug>.txt` and the analogous extraction from `agent/skills/execute-plan/integration-regression-gate.md` to `/tmp/new-<slug>.txt`, then run `diff /tmp/pred-<slug>.txt /tmp/new-<slug>.txt`; confirm exit code 0 and empty output for all six section pairs. (Run before Task 17 deletes the predecessor.)
