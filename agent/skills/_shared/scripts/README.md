...
- **parse-artifact-handoff.py** — Extracts the last `<MARKER>: <path>` line from a subagent's final assistant message and validates the marker family, file existence, non-empty content, and (optionally) path shape. Supported markers: `BRIEF_ARTIFACT`, `SPEC_ARTIFACT`, `PLAN_ARTIFACT`, `REVIEW_ARTIFACT`, `TEST_RESULT_ARTIFACT`. Flags: `--marker <NAME>` (choose from supported markers); `--final-message <path>`; `--expected-path <abs>` (byte-equal match); `--check-existence`; `--check-non-empty`; `--require-path-suffix <SUFFIX>` (e.g., `.md`); `--require-path-prefix <ABS_DIR>` (resolved via `os.path.realpath` on both sides). Example: `python3 parse-artifact-handoff.py --marker SPEC_ARTIFACT --final-message msg.txt --check-existence --check-non-empty --require-path-suffix .md --require-path-prefix /workdir/docs/specs/`.
