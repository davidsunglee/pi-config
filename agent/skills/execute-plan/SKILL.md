...
Emit one evidence block per command-style recipe, with the header `[Evidence for Criterion N]` where `N` is the 1-based criterion number in plan order. Each block contains `command: <exact command>`, `exit_code: <N>`, `stdout:` (fenced), and `stderr:` (fenced), in that order, after the truncation rule. If a criterion has no command-style recipe, no evidence block is emitted for it (gaps in numbering are expected and correct). These blocks are what the orchestrator passes as `{ORCHESTRATOR_COMMAND_EVIDENCE}` in the verifier prompt; the verifier cites them by criterion number (e.g. `evidence: Evidence for Criterion 2`).
