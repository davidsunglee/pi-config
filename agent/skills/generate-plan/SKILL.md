...
- Only exact supported lines count:
  ...
  - `Scout brief: docs/briefs/<filename>` → set `{SCOUT_BRIEF}` to `Scout brief: docs/briefs/<filename>`, **then verify the referenced file exists on disk**:
    ...
    - Staleness classifier (gates planning when source/config drift is detected):
      ...
      - `git rev-parse HEAD failure (HEAD SHA cannot be determined):` Fires when `git rev-parse HEAD` exits non-zero for any reason — e.g., `git` not on PATH, the working directory is not a git repository, repo corruption, or transient I/O error. The current HEAD SHA is unknown, so no enumeration is attempted and no file list is rendered. Capture the failing command's stderr verbatim (or trim leading/trailing whitespace WITHOUT paraphrasing — preserve the literal git error text) and include it as `<error>` in the menu body. Do NOT auto-retry. Surface the following menu to the user:

        > Scout brief at `<path>` was generated at SHA `<brief-sha>`; could not determine current HEAD SHA: `<error>`. Cannot classify intervening changes. The brief may be stale.
        >
        ...
        Apply the shared `Menu response handling` rules below — same `(c)`/`(x)` semantics as the other uninspectable sub-cases. Do not fall through to enumeration.
      ...
      - `Uninspectable sub-case C (git command failure for any other reason):` Fires when the ancestry check passed (sub-case B did not fire) but enumeration still fails — e.g., `git` not on PATH, repo corruption, transient I/O error. Detect by a non-zero exit from `git diff --name-only -z <brief-sha>..HEAD` after `git merge-base --is-ancestor <brief-sha> HEAD` already succeeded. Capture the failing command's stderr verbatim (or trim leading/trailing whitespace WITHOUT paraphrasing — preserve the literal git error text) and include it as `<error>` in the menu body. No file list is rendered (enumeration failed). Do NOT auto-retry the `git diff` enumeration; a single failure surfaces the menu. Surface the following menu to the user:
