...
After a successful commit, the skill offers a three-option continuation menu: `(f) fast lane` (the lightweight workflow at `agent/skills/fastlane/`), `(d) deep workflow` (`generate-plan` → `refine-plan` → `execute-plan`), or `(x) stop`. The orchestrator reads the just-committed spec into context and makes an advisory LLM-native recommendation grounded in actual scope and risk signals (surveyed-file breadth, parser/protocol boundaries, provenance/trust/freshness semantics, cross-skill orchestration, etc.); when uncertain it recommends deep workflow.
The legacy helper `agent/skills/fastlane/scripts/recommend-workflow.py` is retained for compatibility but is no longer authoritative — the user can always override the recommendation by choosing any of the three menu options.

## Continuation menu

After the spec is committed, `define-spec` offers three continuation options to the user. The `fast lane` branch dispatches the [fastlane](../fastlane/README.md) skill for a lightweight single-coder implementation workflow. The deep workflow branch chains `generate-plan` → `refine-plan` → `execute-plan` for comprehensive planning and multi-wave execution. The third option allows the user to stop and return to the command line. The orchestrator reads the just-committed spec and recommends one of the two implementation branches based on real scope and risk signals (cross-skill, parser/protocol, provenance/trust, validation/fallback, git/workflow-boundary, broad orchestration); when uncertain it recommends deep workflow. The user can override the recommendation by choosing either option.
